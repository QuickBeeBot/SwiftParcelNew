...
const NewShipmentForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!from.name.trim()) errors.push(t('newShipment.errors.senderName'));
    if (!from.phone.trim()) errors.push(t('newShipment.errors.senderPhone'));
    if (!from.email.trim() || !/\S+@\S+\.\S+/.test(from.email)) errors.push(t('newShipment.errors.senderEmail'));
    if (!from.address.trim()) errors.push(t('newShipment.errors.senderAddress'));
    if (!from.city.trim()) errors.push(t('newShipment.errors.senderCity'));

    if (!to.name.trim()) errors.push(t('newShipment.errors.recipientName'));
    if (!to.phone.trim()) errors.push(t('newShipment.errors.recipientPhone'));
    if (!to.email.trim() || !/\S+@\S+\.\S+/.test(to.email)) errors.push(t('newShipment.errors.recipientEmail'));
    if (!to.address.trim()) errors.push(t('newShipment.errors.recipientAddress'));
    if (!to.city.trim()) errors.push(t('newShipment.errors.recipientCity'));

    if (!pkg.description.trim()) errors.push(t('newShipment.errors.packageDescription'));
    if (!pkg.weight || parseFloat(pkg.weight) < 0.1) errors.push(t('newShipment.errors.weight'));
    if (!pkg.length || parseInt(pkg.length) < 1) errors.push(t('newShipment.errors.length'));
    if (!pkg.width || parseInt(pkg.width) < 1) errors.push(t('newShipment.errors.width'));
    if (!pkg.height || parseInt(pkg.height) < 1) errors.push(t('newShipment.errors.height'));
    if (pkg.requiresClearance && !pkg.contents.trim()) errors.push(t('newShipment.errors.customsContents'));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitMessage(t('newShipment.success.message', { id: docRef.id }));
      setTimeout(() => navigate('/dashboard/shipments'), 2000);
    } catch (error) {
      console.error('Submission failed:', error);
      setSubmitStatus('error');
      setSubmitMessage(t('newShipment.errors.submitFailed'));
    } finally {
    ...
    <div className="new-shipment-page">
      <div className="form-container">
        <div className="form-header">
          ...
          <h1>{t('newShipment.title')}</h1>
          <p>{t('newShipment.subtitle')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-section">
            <div className="section-title">
              <MapPin size={20} />
              <h3>{t('newShipment.sections.pickupMethod')}</h3>
            </div>
            <div className="radio-group horizontal">
              <label className={`radio-option ${formData.pickupType === 'dropoff' ? 'active' : ''}`}>
                ...
                <div className="radio-content">
                  <strong>{t('newShipment.pickup.dropoffTitle')}</strong>
                  <p>{t('newShipment.pickup.dropoffDesc')}</p>
                </div>
              </label>
              <label className={`radio-option ${formData.pickupType === 'pickup' ? 'active' : ''}`}>
                ...
                />
                <div className="radio-content">
                  <strong>{t('newShipment.pickup.pickupTitle')}</strong>
                  <p>{t('newShipment.pickup.pickupDesc')}</p>
                </div>
              </label>
            </div>
          </div>
          ...
          <div className="form-section">
            <div className="section-title">
              <Truck size={20} />
              <h3>{t('newShipment.sections.addresses')}</h3>
            </div>
            <div className="address-grid">
              <div className="address-card">
                <div className="card-header">
                  <Home size={16} />
                  <h4>{t('newShipment.addresses.sender')}</h4>
                </div>
                <InputField 
                  icon={<Mail size={16} />}
                  placeholder={`${t('newShipment.placeholders.fullName')} *`} 
                  value={formData.from.name} 
                  onChange={(e) => handleChange(e, 'from', 'name')} 
                />
                <InputField 
                  icon={<Phone size={16} />}
                  placeholder={`${t('newShipment.placeholders.phone')} *`} 
                ...
                <div className="location-type-toggle">
                  ...
                  <label>
                    ...
                    />
                    {t('newShipment.addresses.businessLocation')}
                  </label>
                </div>
              </div>
              ...
              <div className="address-card">
                <div className="card-header">
                  <Home size={16} />
                  <h4>{t('newShipment.addresses.recipient')}</h4>
                </div>
                <InputField 
                  icon={<Mail size={16} />}
                  placeholder={`${t('newShipment.placeholders.fullName')} *`} 
          ...
          <div className="form-section">
            <div className="section-title">
              <Package size={20} />
              <h3>{t('newShipment.sections.packageInfo')}</h3>
            </div>
            
            <div className="form-row">
              <label>{t('newShipment.package.categoryLabel')}</label>
              <select 
                value={formData.package.category}
                onChange={(e) => handleChange(e, 'package', 'category')}
                className="category-select"
              >
                <option value="general">{t('newShipment.package.categories.general')}</option>
                <option value="fragile">{t('newShipment.package.categories.fragile')}</option>
                <option value="perishable">{t('newShipment.package.categories.perishable')}</option>
                <option value="hazardous">{t('newShipment.package.categories.hazardous')}</option>
                <option value="documents">{t('newShipment.package.categories.documents')}</option>
              </select>
            </div>

            <TextareaField 
              icon={<FileText size={16} />}
              placeholder={`${t('newShipment.placeholders.packageDescription')} *`} 
            ...
            {formData.package.category === 'hazardous' && (
              <div className="alert warning">
                ...
                {t('newShipment.package.hazardousWarning')}
              </div>
            )}
            ...
            <div className="dimensions-row">
              <InputField 
                type="number" 
                placeholder={`${t('newShipment.placeholders.weight')} *`} 
              ...
              <InputField 
                type="number" 
                placeholder={`${t('newShipment.placeholders.length')} *`} 
              ...
              <InputField 
                type="number" 
                placeholder={`${t('newShipment.placeholders.width')} *`} 
              ...
              <InputField 
                type="number" 
                placeholder={`${t('newShipment.placeholders.height')} *`} 
            ...
            <InputField 
              ...
              placeholder={t('newShipment.placeholders.declaredValue')}
            ...
            <div className="form-row-checkbox">
              <label>
                ...
                />
                {t('newShipment.package.customsClearance')}
              </label>
            </div>
            ...
            {formData.package.requiresClearance && (
              <TextareaField 
                icon={<FileText size={16} />}
                placeholder={t('newShipment.placeholders.customsContents')}
          ...
          <div className="form-section">
            <div className="section-title">
              <Clock size={20} />
              <h3>{t('newShipment.sections.transportSchedule')}</h3>
            </div>
            ...
            <div className="transport-options">
              ...
              <div className="transport-option">
                ...
                <label>
                  ...
                  />
                  {t('newShipment.transport.ground')}
                </label>
              </div>
            </div>
            ...
            <div className="date-row">
              <InputField 
                ...
                type="date" 
                label={t('newShipment.labels.preferredPickupDate')}
              ...
              <InputField 
                icon={<Calendar size={16} />}
                type="date" 
                label={t('newShipment.labels.preferredDeliveryDate')}
            ...
            <div className="form-row">
              <label>{t('newShipment.labels.shipmentType')}</label>
              <select 
                value={formData.shipment.type}
                onChange={(e) => handleChange(e, 'shipment', 'type')}
                className="type-select"
              >
                <option value="standard">{t('newShipment.shipmentTypes.standard')}</option>
                <option value="express">{t('newShipment.shipmentTypes.express')}</option>
                <option value="urgent">{t('newShipment.shipmentTypes.urgent')}</option>
              </select>
            </div>

            <div className="form-row">
              <label>{t('newShipment.labels.contactPreference')}</label>
              <select 
                value={formData.shipment.contactPreference}
                onChange={(e) => handleChange(e, 'shipment', 'contactPreference')}
                className="contact-select"
              >
                <option value="email">{t('newShipment.contactPreferences.email')}</option>
                <option value="sms">{t('newShipment.contactPreferences.sms')}</option>
                <option value="phone">{t('newShipment.contactPreferences.phone')}</option>
              </select>
            </div>

            <TextareaField 
              icon={<MessageSquare size={16} />}
              placeholder={t('newShipment.placeholders.specialInstructions')}
            ...
            <TextareaField 
              icon={<MessageSquare size={16} />}
              placeholder={t('newShipment.placeholders.additionalNotes')}
          ...
          >
            {isSubmitting ? (
              <>
                ...
                {t('newShipment.submitting')}
              </>
            ) : (
              t('newShipment.submitButton')
            )}
          </button>
          ...
          <div className="help-text">
            <p>{t('newShipment.help.reviewTime')}</p>
            <p>{t('newShipment.help.urgentHelp')}</p>
          </div>
        </form>
      </div>
    </div>
...
const InputField = ({ icon, label, placeholder, value, onChange, type = "text", min, step }) => (
...
const TextareaField = ({ icon, label, placeholder, value, onChange, rows = 4 }) => (
  <div className="input-with-icon">
    {icon && <span className="input-icon">{icon}</span>}
    {label && <label>{label}</label>}
    <textarea 
